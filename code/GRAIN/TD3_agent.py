...
class TD3_Agent(object):
	...
	def select_action(self, state):#only used when interact with the env
		with torch.no_grad():
			...
			a = self.actor(state)
			noise = torch.normal(-0.5, 0.5, [a.shape[0], a.shape[1]]) * self.policy_noise
			noise *= 0.8
			a = a + noise
	...
	def train(self,replay_buffer):
		...
		# print(s.shape)
		noise = torch.normal(-0.5, 1, [a.shape[0], a.shape[1]]) * self.policy_noise
		smoothed_target_a = (
...
class ReplayBuffer(object):
	...
	def add(self, state, action, reward, next_state, dead):
		Ti = self.ptr + state.shape[0]
		self.state[self.ptr:Ti] = state
		self.action[self.ptr:Ti] = action
		self.reward[self.ptr] = reward
		self.next_state[self.ptr:Ti] = next_state
		self.dead[self.ptr:Ti] = dead
